...
@app.get("/history/select/{history_id}")
def get_history_for_form(history_id: int, current_user: dict = Depends(get_current_user)):
    """
    Get a specific history item with all form data needed to populate the analyze form
    Returns country_code, state_code, and all states for the country
    """
    try:
        ...
        cursor.execute("SELECT id FROM countries WHERE name = %s", (history_item['country'],))
        country_result = cursor.fetchone()
        country_code = str(country_result['id']) if country_result else ""
        
        # Get all states for this country
        states = []
        state_code = ""
        if country_code:
            cursor.execute("SELECT id, name FROM states WHERE country_id = %s", (country_code,))
            states_result = cursor.fetchall()
            states = [{"id": str(row['id']), "name": row['name']} for row in states_result]
            
            # If this is a state report, find the state code
            if history_item['state'] and history_item['report_type'] == 'state':
                cursor.execute("SELECT id FROM states WHERE name = %s AND country_id = %s", 
                             (history_item['state'], country_code))
                state_result = cursor.fetchone()
                state_code = str(state_result['id']) if state_result else ""
        ...
        return {
            ...
            "report_type": history_item['report_type'],
